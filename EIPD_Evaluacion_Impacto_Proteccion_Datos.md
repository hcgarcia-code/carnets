...
- **[v1.2]** Identificación fiable del origen de cada petición (`gestion/ip.py`). Los limitadores de intentos y el registro de auditoría usaban `REMOTE_ADDR`, que detrás del proxy inverso de producción era siempre la dirección del balanceador. Dos consecuencias, ambas verificadas sobre el registro real: el limitador de reposición de contraseña, que cuenta solo por dirección, era de 5 peticiones cada 15 minutos **para la totalidad del servicio**, y el campo `ip` del registro de auditoría repetía el mismo valor en todas las líneas, quedando inservible para investigar un incidente. Además, la dirección que se guardaba como constancia de la firma del acuerdo (`ip_aceptacion`) se tomaba de la primera entrada de `X-Forwarded-For`, que es la que puede escribir el propio firmante. Ahora la cabecera solo se atiende cuando la petición procede de un proxy declarado en `PROXIES_DE_CONFIANZA`, y se recorre de derecha a izquierda.
